...
use crate::file_shell::{
    file_table::file_table::FileTable, root_file::error::FileSystemOperationError, user::access_key::AccessKey,
};

// use super::helper::get_ft_guard;

pub fn write(file_path: &str, content: String,ackey:AccessKey) -> Result<usize, FileSystemOperationError> {
    let ft = get_ft()?;
    let ft = ft.lock();
