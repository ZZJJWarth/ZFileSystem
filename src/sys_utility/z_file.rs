...
impl ZFile {
    ...
    fn raw_write(&mut self,offset: u32, buf: &Vec<u8>, size: u32)->Result<(),()>{
        ...
        let max_len=self.block_servant.write_check(max_len, offset, size).unwrap();
    ...
    pub fn file_to_u8(file:ZFile)->[u8;ZFILE_SIZE]{
        let file=unsafe {
            transmute::<ZFile,[u8;ZFILE_SIZE]>(file)
        };
        file
    }
    ...
    pub fn u8_to_file(v:[u8;ZFILE_SIZE])->ZFile{
        let v=unsafe{
            transmute::<[u8;ZFILE_SIZE],ZFile>(v)
        };
        v
    }

    pub fn char_to_u8()->u8{todo!()}
