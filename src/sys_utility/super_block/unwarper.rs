...
use crate::{SUPER_BLOCK, file_shell::root_file::error::FileSystemOperationError, sys_utility::bitmap::block_bit_map::BlockBitmap};
...
pub fn unwrap_bitmap<'a>(input:&'a Option<Arc<Mutex<BlockBitmap>>>)->Result<std::sync::MutexGuard<'a, crate::sys_utility::bitmap::block_bit_map::BlockBitmap>,FileSystemOperationError>{
    ...
    let bitmap_ptr=match input{
        Some(s)=>s,
        None=>{return Err(FileSystemOperationError::InitError(format!("初始化未完成")));}
    };
    let bitmap=match bitmap_ptr.lock(){
        Ok(l)=>l,
        Err(_)=>{return Err(FileSystemOperationError::InitError(format!("初始化未完成")));}
    };
    Ok(bitmap)
    // todo!()
}