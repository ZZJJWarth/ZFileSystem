use std::{fs::File, io::{BufWriter, Seek},io::{Write, BufReader, Read, BufRead}, mem::transmute};

use crate::{sys_utility::addr::addr::BlockAddr, file_shell::root_file::error::FileSystemOperationError};
...
impl SuperPack{
    pub fn dump(self,path:&str)->Result<(),FileSystemOperationError>{
        ...
        let f=match f{
            Ok(s)=>s,
            Err(_)=>{return Err(FileSystemOperationError::DiskError(format!("未找到磁盘")));}
        };
        let mut bw=BufWriter::new(f);
        bw.seek(std::io::SeekFrom::Start(0));
        let buf=unsafe {
            transmute::<Self,[u8;SuperBlock::SUPER_BLOCK_SIZE as usize]>(self)
        };
        bw.write(&buf);
        Ok(())
    }
    ...
    pub fn load(path:&str)->Result<Self,FileSystemOperationError>{
        ...
        let mut buf:[u8;SuperBlock::SUPER_BLOCK_SIZE as usize]=[0;SuperBlock::SUPER_BLOCK_SIZE as usize];
        br.fill_buf();
        let bu=br.buffer();
        for i in 0..bu.len(){
            buf[i]=bu[i];
